...
if (isset($_GET['reserva'])) {
    if ($_SESSION['usuario']) {
        ...
        header('Location: eliminar_reserva.php?reserva=' . $reserva);
    }
}
...
<div class="body">
    <form method="post" action="<?php echo htmlspecialchars($_SERVER[" PHP_SELF"]); ?>">
        ...
        <div class="form__container">
            ...
            <div class="google">
                <div class="g_id_signin" data-type="standard" data-shape="rectangular" data-theme="outline" data-text="signin_with" data-size="large" data-locale="es_ES">
                </div>
                <div id="g_id_onload" data-client_id="<?php echo $client_id; ?>" data-login_uri="<?php echo $redirect_uris; ?>" data-auto_prompt="false">
                </div>
            </div>
        </div>
...
<script>
    function onSignIn(googleUser) {
        ...
        // Comprueba si la URL contiene la palabra "reserva"
        if (window.location.href.indexOf('reserva') !== -1) {
            // Extrae el parámetro "reserva" de la URL
            const urlParams = new URLSearchParams(window.location.search);
            const reserva = urlParams.get('reserva');

            // Redirige a eliminar_reserva.php con el parámetro de reserva
            window.location.href = `eliminar_reserva.php?reserva=${reserva}`;
        } else {
            // Si no hay "reserva" en la URL, redirige a otra página o maneja el inicio de sesión como desees
